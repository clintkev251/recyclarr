#!/bin/bash
export CONFIG=/etc/trash/config
cd /etc/trash

# Test if config exists. If not, create it and exit
if test ! -f "$CONFIG/recyclarr.yml"; then
...
if test -n "$CRON_EXPRESSION"; then
    ...
    if $RUN_RADARR = "true"; then
        ...
        echo "$CRON_EXPRESSION /etc/trash/exec radarr --config $CONFIG/recyclarr.yml >/proc/1/fd/1 2>/proc/1/fd/2" | crontab
    fi
    if $RUN_SONARR = "true"; then
        echo "Setting up job for Sonarr"
        echo "$CRON_EXPRESSION /etc/trash/exec sonarr --config $CONFIG/recyclarr.yml >/proc/1/fd/1 2>/proc/1/fd/2" | crontab
    fi
    echo "Starting cron"
    exec cron -f
else
    ...
    if $RUN_RADARR = "true"; then
        ./exec radarr --config $CONFIG/recyclarr.yml
    fi
    if $RUN_SONARR = "true"; then
        ./exec sonarr --config $CONFIG/recyclarr.yml
    fi
fi
